...
import type { GameData, GamePageData, HLTBGameStats, SearchResults } from "./GameInfoData"
...
// NOTE: Copied from https://github.com/morwy/hltb-for-deck/blob/main/src/hooks/HltbApi.ts which is itself a
// reproduction of https://github.com/ScrappyCocco/HowLongToBeat-PythonAPI/pull/26
      ...
      for (const script of scripts) {
        if (script.src.includes("_app-")) {
          ...
          if (scriptResponse.status === 200) {
            ...
            const pattern = /\/api\/[a-zA-Z]*\/"\.concat\("([a-zA-Z0-9]+)"\)\.concat\("([a-zA-Z0-9]+)"\)/
            ...
            if (matches?.[1]) {
              ...
              searchApiRoot = matches[0].split(".")[0].slice(0, -1)
...
async function fetchNextJsKey() {
  try {
    ...
    if (response.status === 200) {
      ...
      for (const script of scripts) {
        if (script.src.includes("_ssgManifest.js") || script.src.includes("_buildManifest.js")) {
...
const keyCache = new Map<symbol, { key: string | null; keyFetch: () => Promise<string | null> }>([
  [SearchKey, { key: null, keyFetch: fetchSearchKey }],
  [NextJsKey, { key: null, keyFetch: fetchNextJsKey }],
])
async function fetchWithKey(keyName: symbol, callback: (key: string) => Promise<Response>) {
  const cache = keyCache.get(keyName)
  if (!cache) {
...
async function fetchSearchResults(appName: string) {
  const response = await fetchWithKey(SearchKey, (key) => fetchSearchResultsWithKey(appName, key))
  if (!response) {
    // Error already logged
    return null
  }
...
async function fetchMostCompatibleGameData(appName: string, appId?: number, hltbGameId?: number) {
...
export async function fetchHltbGameStats(
  ...
  try {
    ...
    if (gameData) {
      return {
        mainStat: gameData.comp_main > 0 ? (gameData.comp_main / 60 / 60).toFixed(1) : "--",
        mainPlusStat: gameData.comp_plus > 0 ? (gameData.comp_plus / 60 / 60).toFixed(1) : "--",
        completeStat: gameData.comp_100 > 0 ? (gameData.comp_100 / 60 / 60).toFixed(1) : "--",
        allStylesStat: gameData.comp_all > 0 ? (gameData.comp_all / 60 / 60).toFixed(1) : "--",
